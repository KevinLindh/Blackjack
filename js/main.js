  ...
  fetch(url)
      ...
      .then(data => {
        ...
        if(val1 > val2){
          ...
          let win = Number(localStorage.getItem("credits")) + 100;
          localStorage.setItem("credits", win);
          document.querySelector("#credits").innerText = `Credits: ${localStorage.getItem("credits")}`
        }else if(val1 < val2){
          document.querySelector('h3').innerText = 'Player 2 WON!'
          let lose = Number(localStorage.getItem("credits")) - 100;
            if(lose <= 0){
                localStorage.setItem("credits", 0);
                document.querySelector("#credits").innerText = `Credits: ${localStorage.getItem("credits")}`
                document.querySelector("#addCredits").innerText = "You lost and have no credits left! Add 1000 more?"
                document.querySelector("#dealCards").disabled = true;
                document.querySelector("#addCredits").classList.remove("hide")
              } else{
                localStorage.setItem("credits", lose);
                document.querySelector("#credits").innerText = `Credits: ${localStorage.getItem("credits")}`
            }
        }else{
